...
vk::raii::Instance& Device::getInstance()
{
    return instance;
}
vk::raii::SurfaceKHR& Device::getSurface()
{
    return surface;
}
...
vk::SampleCountFlagBits Device::getMsaaSamples() const
{
    return msaaSamples;
}
vk::SampleCountFlagBits Device::getMaxMsaaSamples() const
{
    return maxMsaaSamples;
}
vk::SampleCountFlags Device::getSupportedMsaaSamples() const
{
...
{
    constexpr vk::ApplicationInfo appInfo{.pApplicationName   = "Blaz-Vulkan",
    ...
    auto unsupportedLayerIt =
        std::ranges::find_if(validationLayers,
...
void Device::createLogicalDevice()
{
    ...
    VpDeviceCreateInfo vpDeviceCreateInfo{.pCreateInfo =
                                              reinterpret_cast<const VkDeviceCreateInfo*>(
                                                  &deviceCreateInfo),
    ...
    if (vpCreateDevice(*physicalDevice, &vpDeviceCreateInfo, nullptr, &rawDevice) != VK_SUCCESS)
        throw std::runtime_error(
            "failed to create logical device with VP_KHR_ROADMAP_2022 profile!");
...
bool Device::isDeviceSuitable(const vk::raii::PhysicalDevice& device)
{
    ...
    return profileSupported && supportsGraphics && supportsAllRequiredExtensions && isDiscreteGPU;
}
...
vk::raii::ImageView Device::createImageView(vk::Image image,
                                            vk::Format format,
                         ...
                         vk::raii::DeviceMemory& imageMemory)
{
    ...
    vk::MemoryAllocateInfo allocInfo{.allocationSize = memReqs.size,
                                     .memoryTypeIndex =
...
{
    vk::CommandBufferAllocateInfo allocInfo{.commandPool        = uploadCommandPool_,
...
void Device::copyBufferToImage(const vk::raii::Buffer& buffer,
                               vk::raii::Image& image,
                               uint32_t width,
                               uint32_t height)
{
