...
void VulkanRenderer::createDescriptorSetLayout()
{
    std::array bindings = {vk::DescriptorSetLayoutBinding(0,
                                                          ...
                                                          nullptr), // Uniform Buffer Objects
                           vk::DescriptorSetLayoutBinding(1,
                                                          vk::DescriptorType::eCombinedImageSampler,
                                                          TextureManager::MAX_TEXTURES,
                                                          vk::ShaderStageFlagBits::eFragment,
                                                          nullptr), // Bindless Textures
    ...
    vk::DescriptorSetLayoutCreateInfo layoutInfo{.pNext = &bindingFlagsInfo,
                                                 .bindingCount =
                                                     static_cast<uint32_t>(bindings.size()),
                                                 .pBindings = bindings.data()};
    descriptorSetLayout =
        vk::raii::DescriptorSetLayout(vulkanDevice->getLogicalDevice(), layoutInfo);
}
...
[[nodiscard]] vk::raii::ShaderModule VulkanRenderer::createShaderModule(
...
{
    vk::raii::ShaderModule shaderModule        = createShaderModule(readFile("shaders/scene.spv"));
    ...
    vk::PipelineDynamicStateCreateInfo dynamicState{
        ...
        .pDynamicStates    = dynStates.data()}; // Viewport and Scissor are marked as dynamic so the
                                             // pipeline doesn't need recreation on resize
    vk::PipelineInputAssemblyStateCreateInfo inputAssembly{
        .topology = vk::PrimitiveTopology::eTriangleList};
    vk::PipelineViewportStateCreateInfo viewportState{.viewportCount = 1, .scissorCount = 1};
    vk::PipelineRasterizationStateCreateInfo rasterizer{.polygonMode = vk::PolygonMode::eFill,
                                                        .cullMode    = vk::CullModeFlagBits::eBack,
                                                        .frontFace =
                                                            vk::FrontFace::eCounterClockwise,
                                                        .lineWidth = 1.0f};
    vk::PipelineMultisampleStateCreateInfo multisampling{.rasterizationSamples = msaaSamples};
    vk::PipelineColorBlendAttachmentState colorBlendAtt{.blendEnable = vk::False,
                                                        .colorWriteMask =
    ...
    std::array<vk::Format, 2> colorFormats = {swapchain->getSurfaceFormat().format,
...
{
    vk::raii::ShaderModule shaderModule = createShaderModule(readFile("shaders/shadow.spv"));
    vk::PipelineShaderStageCreateInfo vertStage{.stage  = vk::ShaderStageFlagBits::eVertex,
                                                .module = shaderModule,
                                                .pName  = "vertMain"};

    auto bindingDesc = getVertexBindingDescription();
    vk::VertexInputAttributeDescription posAttr{0,
                                                0,
    ...
    vk::Format depthFmt = vulkanDevice->findDepthFormat();
    vk::PipelineRenderingCreateInfo renderingInfo{.colorAttachmentCount  = 0,
                                                  .depthAttachmentFormat = depthFmt};
...
void VulkanRenderer::createSkyPipeline()
{
    ...
    vk::PushConstantRange pushRange{.stageFlags = vk::ShaderStageFlagBits::eFragment,
                                    .offset     = 0,
    ...
    vk::PipelineDynamicStateCreateInfo dynState{.dynamicStateCount =
    ...
    vk::PipelineColorBlendAttachmentState colorBlendAtt{.blendEnable = vk::False,
                                                        .colorWriteMask =
                                                            ...
                                                            vk::ColorComponentFlagBits::eA};
    std::array<vk::PipelineColorBlendAttachmentState, 2> blendAtts = {colorBlendAtt, colorBlendAtt};
    vk::PipelineColorBlendStateCreateInfo colorBlending{.attachmentCount =
                                                            uint32_t(blendAtts.size()),
                                                        .pAttachments = blendAtts.data()};
    vk::PipelineDepthStencilStateCreateInfo depthStencil{.depthTestEnable  = vk::True,
...
{
    vk::DescriptorPoolSize uboPoolSize{.type            = vk::DescriptorType::eUniformBuffer,
    ...
    skyDescriptorSets = vulkanDevice->getLogicalDevice().allocateDescriptorSets(
        vk::DescriptorSetAllocateInfo{.descriptorPool     = skyDescriptorPool,
...
void VulkanRenderer::createColorResources()
{
    ...
    vulkanDevice->createImage(swapchain->getExtent().width,
                              ...
                              colorImageMemory);
    colorImageView =
        vulkanDevice->createImageView(colorImage, colorFormat, vk::ImageAspectFlagBits::eColor, 1);
}
...
void VulkanRenderer::createDepthResources()
{
    ...
    vulkanDevice->createImage(swapchain->getExtent().width,
                              ...
                              depthImageMemory);
    depthImageView =
        vulkanDevice->createImageView(depthImage, depthFormat, vk::ImageAspectFlagBits::eDepth, 1);
}

void VulkanRenderer::createShadowMapResources()
{
    vk::Format depthFormat = vulkanDevice->findDepthFormat();
    vulkanDevice->createImage(SHADOW_MAP_SIZE,
                              SHADOW_MAP_SIZE,
                              1,
                              vk::SampleCountFlagBits::e1,
                              depthFormat,
...
void VulkanRenderer::createNormalResources()
{
    ...
    vulkanDevice->createImage(swapchain->getExtent().width,
                              ...
                              vk::ImageTiling::eOptimal,
                              vk::ImageUsageFlagBits::eColorAttachment |
                                  (msaaEnabled ? vk::ImageUsageFlagBits{}
                                               : vk::ImageUsageFlagBits::eSampled),
                              vk::MemoryPropertyFlagBits::eDeviceLocal,
                              normalImage,
                              normalImageMemory);
    normalImageView = vulkanDevice->createImageView(normalImage,
                                                    normalFormat,
    ...
    if (msaaEnabled)
    {
        vulkanDevice->createImage(swapchain->getExtent().width,
                                  ...
                                  vk::MemoryPropertyFlagBits::eDeviceLocal,
                                  normalResolveImage,
                                  normalResolveImageMemory);
        normalResolveImageView = vulkanDevice->createImageView(normalResolveImage,
                                                               normalFormat,
                                                               vk::ImageAspectFlagBits::eColor,
                                                               1);
    }
...
void VulkanRenderer::uploadRenderable(Renderable& r,
...
void VulkanRenderer::loadScene()
{
    ...
    if (scene.contains("skybox"))
    {
        ...
        if (s.contains("horizonColor"))
            settings_.skyPush.horizonColor = {s["horizonColor"][0],
                                              s["horizonColor"][1],
                                              s["horizonColor"][2],
                                              1.0f};
        if (s.contains("zenithColor"))
            settings_.skyPush.zenithColor = {s["zenithColor"][0],
                                             s["zenithColor"][1],
                                             s["zenithColor"][2],
                                             1.0f};
        if (s.contains("groundColor"))
            settings_.skyPush.groundColor = {s["groundColor"][0],
                                             s["groundColor"][1],
                                             s["groundColor"][2],
                                             1.0f};
        if (s.contains("sunColor"))
            settings_.skyPush.sunParams = {s["sunColor"][0],
                                           s["sunColor"][1],
                                           s["sunColor"][2],
    ...
    for (const auto& obj : scene["objects"])
    {
        ...
        {
            return obj.contains(key) ? std::optional<std::string>{obj[key].get<std::string>()}
                                     : std::nullopt;
        };

        glm::vec3 rotDeg =
            obj.contains("rotation")
        ...
        else
        {
            ...
            if (ext != ".gltf" && ext != ".glb")
                throw std::runtime_error("unsupported mesh format '" + mesh +
                                         "' — only .gltf/.glb are supported");
            ...
            for (auto& prim : loadGLTF(assetPath(mesh), yUpToZUp))
            {
                ...
                {
                    if (!img.path.empty())
                        return textureManager_.load(img.path, linear);
                    if (!img.bytes.empty())
                        return textureManager_.loadFromMemory(img.bytes, linear);
                    return 0xFFFFu;
                };
                Renderable r;
                r.label                  = mesh;
        ...
        r.textureIndex =
            optStr("texture") ? textureManager_.load(assetPath(*optStr("texture"))) : 0xFFFFu;
        r.metallicRoughnessIndex = optStr("specularMap")
                                       ? textureManager_.load(assetPath(*optStr("specularMap")))
                                       : 0xFFFFu;
        r.normalMapIndex         = optStr("normalMap")
                                       ? textureManager_.load(assetPath(*optStr("normalMap")), true)
                                       : 0xFFFFu;
        r.heightMapIndex         = optStr("heightMap")
                                       ? textureManager_.load(assetPath(*optStr("heightMap")), true)
                                       : 0xFFFFu;
...
void VulkanRenderer::createDescriptorSets()
{
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
    {
        ...
        vk::DescriptorImageInfo shadowInfo{*shadowMapSampler,
                                           *shadowMapImageView,
...
void VulkanRenderer::createSyncObjects()
{
    ...
    for (size_t i = 0; i < swapchain->getImages().size(); ++i)
        renderFinishedSemaphores.emplace_back(vulkanDevice->getLogicalDevice(),
                                              vk::SemaphoreCreateInfo());

    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
    {
        presentCompleteSemaphores.emplace_back(vulkanDevice->getLogicalDevice(),
                                               vk::SemaphoreCreateInfo());
        inFlightFences.emplace_back(vulkanDevice->getLogicalDevice(),
...
void VulkanRenderer::drawFrame()
{
    ...
    auto fenceResult = vulkanDevice->getLogicalDevice().waitForFences(*inFlightFences[frameIndex],
                                                                      vk::True,
                                                                      UINT64_MAX);
    ...
    auto [result, imageIndex] =
        swapchain->getSwapChain().acquireNextImage(UINT64_MAX,
...
void VulkanRenderer::updateUniformBuffer(uint32_t currentImage)
{
    ...
    glm::vec3 lightPos{glm::cos(settings_.lightAngle) * 8.0f,
                       glm::sin(settings_.lightAngle) * 8.0f,
                       6.0f};
    ...
    glm::mat4 lightView =
        glm::lookAt(frustumCenter +
                        lightDir * settings_.shadowFar, // light is upstream of the frustum
                    frustumCenter,
    ...
    ubo.materialParams = glm::vec4(settings_.ambient,
                                   settings_.defaultRoughness,
    ...
    {
        if (!pl.enabled || activeLights >= RenderSettings::MAX_POINT_LIGHTS)
            continue;
    ...
    ubo.pomParams =
        glm::vec4(settings_.pomDepthScale, settings_.pomMinSteps, settings_.pomMaxSteps, 0);
    ...
    ubo.fogParams =
        glm::vec4(effectiveDensity, settings_.fogHeightFalloff, settings_.fogMaxOpacity, 0);
...
void VulkanRenderer::recordCommandBuffer(uint32_t imageIndex)
{
    ...
    vk::RenderingAttachmentInfo shadowDepthAtt{.imageView = *shadowMapImageView,
                                               .imageLayout =
                                                   vk::ImageLayout::eDepthAttachmentOptimal,
    ...
    vk::RenderingAttachmentInfo colorAtt =
        msaaEnabled
            ...
            : vk::RenderingAttachmentInfo{.imageView   = *swapchain->getImageViews()[imageIndex],
                                          .imageLayout = vk::ImageLayout::eColorAttachmentOptimal,
                                          .loadOp      = vk::AttachmentLoadOp::eClear,
                                          .storeOp     = vk::AttachmentStoreOp::eStore,
                                          .clearValue  = clearColor};

    vk::RenderingAttachmentInfo depthAtt{.imageView   = *depthImageView,
    ...
    vk::RenderingAttachmentInfo normalAtt =
        msaaEnabled
            ? vk::RenderingAttachmentInfo{.imageView   = *normalImageView,
                                          .imageLayout = vk::ImageLayout::eColorAttachmentOptimal,
                                          .resolveMode = vk::ResolveModeFlagBits::eAverage,
    ...
    if (settings_.skyEnabled)
    {
        ...
        cmd.pushConstants2(vk::PushConstantsInfo{}
                               ...
                               .setPValues(&settings_.skyPush));
        cmd.draw(3, 1, 0, 0);
    }
    cmd.endRendering();

    // Barrier before ImGui pass so it sees finished color writes.
    vk::MemoryBarrier2 memBarrier{.srcStageMask =
                                      vk::PipelineStageFlagBits2::eColorAttachmentOutput,
    ...
    cmd.pipelineBarrier2(
        vk::DependencyInfo{.memoryBarrierCount = 1, .pMemoryBarriers = &memBarrier});
...
void VulkanRenderer::recreateSwapChain()
{
    ...
    ImGui_ImplVulkan_SetMinImageCount(static_cast<uint32_t>(swapchain->getImages().size()));
    createDepthResources();
    createColorResources();
    createNormalResources();
}
